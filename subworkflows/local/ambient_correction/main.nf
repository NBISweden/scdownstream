...
workflow AMBIENT_CORRECTION {
    ...
    if (method == 'none') {
        ...
        ch_h5ad = ch_h5ad.mix(ch_multi.input.map { meta, filtered, _unfiltered -> [meta, filtered] })
    }
    else if (method == 'decontx') {
        CELDA_DECONTX(ch_multi.input, ch_multi.batch_col, ch_multi.input_layer, ch_multi.output_layer)
        ...
        CELLBENDER_REMOVEBACKGROUND(ch_multi.input.map { meta, _filtered, unfiltered -> [meta, unfiltered] })
        ...
        SOUPX(ch_multi.input, 0.8, ch_multi.input_layer, ch_multi.output_layer)
